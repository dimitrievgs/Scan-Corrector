...
namespace Tester.Controls
{
    public class ZoomPictureBox : UserControl
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (mouseState == MouseState.Drag || Zoom < 1f)
            {
                e.Graphics.InterpolationMode = InterpolationMode.NearestNeighbor;
            }

            var dx = -visibleCenter.X * Zoom + ClientSize.Width / 2f;
            var dy = -visibleCenter.Y * Zoom + ClientSize.Height / 2f;

            e.Graphics.DrawImage(image, dx, dy, image.Width * Zoom, image.Height * Zoom);
